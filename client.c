...
int main() {
    ...
    client_host = socket(AF_INET, SOCK_STREAM, 0);
    ...
    connect(client_host, (struct sockaddr *)&server_address, sizeof(server_address));
    //connect(client_guess, (struct sockaddr *)&server_address, sizeof(server_address));

    // Get the role you have: You joined first -> Word Setter.
    memset(buffer, 0, sizeof(buffer));
    read(client_host, buffer, BUFFER_SIZE);
    ...
    if (strcmp(buffer, "Word Setter") == 0) {
        ...
        word[strcspn(word, "\n")] = 0;
        //Sending
        send(client_host, word, WORD_LEN, 0);
        ...
        //     send(client_guess, guess, WORD_LEN, 0);

        //     memset(word, 0, sizeof(word));
        //     read(client_guess, word, WORD_LEN);
        //     word[WORD_LEN] = '\0';
        //     printf("Feedback: %s\n", word);
        //     memset(feedback, 0, sizeof(feedback));
        //     read(client_guess, feedback, WORD_LEN);
        //     feedback[WORD_LEN] = '\0';
        //     printf("Feedback: %s (G=correct, Y=wrong pos, B=wrong letter)\n", feedback);

        //     memset(result, 0, sizeof(result));
        //     read(client_guess, result, sizeof(result));
        ...
        if(strcmp(buffer, "Ready") == 0) {
            ...
            for (int i = 0; i < MAX_GUESSES; i++) {
                ...
                send(client_host, guess, WORD_LEN, 0);
                ...
                printf("Feedback: %s\n", word);
                memset(feedback, 0, sizeof(feedback));
                read(client_host, feedback, WORD_LEN);
                feedback[WORD_LEN] = '\0';
                printf("Feedback: %s (G=correct, Y=wrong pos, B=wrong letter)\n", feedback);

                memset(result, 0, sizeof(result));
                read(client_host, result, sizeof(result));
                result[sizeof(result)] = '\0';
